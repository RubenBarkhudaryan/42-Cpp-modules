...
class	BitcoinExchange
{
	private:
		...
		std::list<std::string>		split(const std::string& str, char target);
		bool						isLeap(int year);
		bool						isValidDate(const std::string& date);
		bool						isValidPair(const std::list<std::string>& splited);
		bool						isTitle(const std::list<std::string>& line);
		void						addValue(
